// app/api/products/list_by_bi/route.ts
...
export async function POST(req: Request) {
  ...
  const { data, error } = await supabase.rpc("product_list_by_bi", {
    bi_list,
    lim: limit,
    off: offset,
  });
  if (error) {
    return NextResponse.json({ error: "RPC error: product_list_by_bi" }, { status: 500 });
  }

  // data è un array di { id: uuid }
