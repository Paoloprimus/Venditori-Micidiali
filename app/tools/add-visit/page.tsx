...
export default function AddVisitPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Le note NON vengono cifrate perché l'AI deve leggerle per fare suggerimenti
      // Aggiungiamo una nuova colonna "notes" in chiaro invece di note_conversazione_enc
      if (form.note.trim()) {
...
export default function AddVisitPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Cifra note se presenti
      if (form.note.trim() && crypto?.crypto) {
        try {
          const encrypted = await (crypto.crypto as any).encryptFields(
            "table:visits",
            "visits",
            '',
            { note_conversazione: form.note.trim() }
          );

          if (Array.isArray(encrypted) && encrypted.length > 0) {
            const noteEnc = encrypted.find((f: any) => f.name === 'note_conversazione');
            if (noteEnc) {
              payload.note_conversazione_enc = noteEnc.ciphertext;
              payload.note_conversazione_iv = noteEnc.iv;
            }
          }
        } catch (err) {
          console.error('Encryption error:', err);
          alert('Errore nella cifratura delle note');
          setBusy(false);
          return;
        }
      }
      ...
      <div style={{ paddingTop: 70, padding: '70px 16px 16px', maxWidth: 600, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          <div>
            ...
            />
            <div style={{ fontSize: 12, color: '#6b7280', marginTop: 4 }}>
              🔒 Le note vengono cifrate per privacy
            </div>
          </div>
