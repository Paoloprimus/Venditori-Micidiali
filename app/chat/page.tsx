...
export type ConversationState = {
  ...
  topic_attivo: LocalScope | null;    // alias storico, può essere null
...
type ConversationContextValue = {
  ...
  setTopic: (t: LocalScope | null) => void;
...
export function ConversationProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        let nextScope: LocalScope = (parsed.scope as LocalScope) ?? DEFAULT_SCOPE;
  ...
  const api: ConversationContextValue = {
    ...
    setScope: (s) =>
      setState((st) => {
        ...
        return {
          ...st,
          scope: s,
          topic_attivo: s, // mantieni allineati alias quando scegli esplicitamente lo scope
          updated_at: ts,
          lastUpdateTs: ts,
        };
      }),
    ...
    remember: (patch) =>
      setState((st) => {
        const ts = now();
        // proteggi sempre lo scope: normalizza eventuali stringhe legacy
